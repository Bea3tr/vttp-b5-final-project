...
import { Comment, MediaFile, PfResult, Post, UserInfo } from '../models/models';
...
export class ProfileComponent implements OnInit {
  ...
  constructor(private matIconRegistry: MatIconRegistry, private domSanitizer: DomSanitizer,
    ...
    const icons = ["shelter", "shop", "comment", "cross", "thumbsup", "thumbsdown", "send"];
    icons.forEach((icon) => {
  ...
  switchTab(tab: string) {
    this.profileSvc.reloadActiveTab(tab);
  }
  ...
  onFileChange(event: any) {
    ...
    this.profileSvc.updatePic(this.selectedFile, this.id)
      .then((resp) => {
        alert(resp.message);
      })
      .catch((err) => {
  ...
  editUsername() {
    ...
    this.profileSvc.updateName(name, this.id)
      .then((resp) => {
        alert(resp.message);
      })
      .catch((err) => {
  ...
  editPassword() {
    this.profileSvc.updatePassword(this.pwForm, this.id)
      .then((resp) => {
        alert(resp.message);
      })
      .catch((err) => {
  ...
  private createUserForm(): FormGroup {
    ...
    return this.fb.group({
      name: this.fb.control<string>(this.name, [Validators.required, Validators.minLength(3)])
    });
  }
